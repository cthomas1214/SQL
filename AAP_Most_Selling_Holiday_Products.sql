...
SELECT TOP 100 ac.ProdOpQty, ac.SumPrdQty, ac.ProdNameQty, tqp.SumProductQuantity AS TotalQuantity, ac.ProdIdQty AS ProductId
FROM AccurateQuantities ac
JOIN TotalQTYFullProds tqp
ON ac.ProdIdQty = tqp.FProductId
JOIN ProductCategories pc
ON ac.ProdIdQty = pc.ProductId
JOIN Products P
ON ac.ProdIdQty = P.ProductId

WHERE pc.CategoryId = 32 AND P.IsDisabled = 0 AND P.ProductId > 964
